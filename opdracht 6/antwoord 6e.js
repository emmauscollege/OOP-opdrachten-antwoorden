...
class Dierentuin {
  ...
  voegDierToe(nieuwDier) {
    if (this.#dieren.includes(nieuwDier) == false) {
      this.#dieren.push(nieuwDier);
    }
  }

  voegVerzorgerToe(nieuweVerzorger) {
    if (this.#verzorgers.includes(nieuweVerzorger) == false) {
      this.#verzorgers.push(nieuweVerzorger);
    }
  }

  laatAlleDierenGeluidMaken() {
    for (var i = 0; i < this.#dieren.length; i++) {
      console.log(this.#dieren[i].naam + " zegt: " + this.#dieren[i].geluidMaken());
    }
  }
}
